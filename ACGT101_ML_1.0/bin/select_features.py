...
def selected_feature(X, y, output_path):
    ...
    plt.plot(range(1, X.shape[1]+1, 1), score_rf, color='blue')
    plt.xticks(range(1, X.shape[1]+1, 1), fontsize=10)
    plt.yticks(fontsize=10)
    plt.title("Random Forest Recursive Feature Elimination Method to Find the Optimal Number of Features", fontdict=fontdict_title)
    plt.savefig(f"{output_path}/randomforest_features_curve.jpg")
    ...
    plt.figure(figsize=[20, 5], dpi=300)
    plt.plot(range(1, X.shape[1]+1, 1), score_svm, color='blue')
    plt.xticks(range(1, X.shape[1]+1, 1), fontsize=10)
    plt.yticks(fontsize=10)
    plt.title("Support Vector Machine Recursive Feature Elimination Method to Find the Optimal Number of Features", fontdict=fontdict_title)
    plt.savefig(f"{output_path}/SVM_features_curve.jpg")
...
def select_lasso(X, y, output_path, alpha=0.05):
    la = Lasso(alpha)
    la.fit(X, y)
    ...
    if len(keys)<7:
        ...
        figsize=len(keys)-1
    plt.figure(figsize=(figsize,7),dpi=300)
    ...
    plt.savefig(f"{output_path}/select_lasso.jpg")
